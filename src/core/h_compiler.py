...
class Generator(object):
    ...
    def __init__(self, env=None):
        #init standard types
        self.types = [
            'int', 'float', 'bool', 'char', 'string', 'auto', 'File',
            'JSONValue'
        ]
        ...
        # init functions list
        self.funcs = [
            'print', 'write', 'ReadStr', 'ReadInt', 'ReadFloat', 'ReadChar',
            'sin', 'cos', 'tan', 'PI', 'fmax', 'fmin', 'abs', 'exit', 'to_int',
            'to_string', 'to_float', 'RemoveFile', 'ReadFromFile',
            'WriteToFile', 'CloseFile', 'GetYear', 'GetMonth', 'GetDay',
            'GetHour', 'GetMinute', 'GetSecond', 'download', 'upload', 'get',
            'post', 'isAlpha', 'isAlphaNum', 'isDigit', 'isUpper', 'isWhite',
            'toLower', 'toUpper', 'ShellCommand', 'ExcuteCommand', 'isFile',
            'isDir', 'rmdir', 'mkdir', 'getcwd', 'thisExePath', 'listdir',
            'split', 'parseJSON', 'SysPlatform', 'RandomNumber'
        ]
        ...
        if self.test(tree):
            ...
            if use:
                ...
                if getenv('HPATH') != None:
                    hascal_runtime_path = str(
                        getenv('HPATH') + "\\hlib" + "\\rt.d")
                    hascal_runtime = ""
                    ...
                    HascalException(
                        "Error : 'HPATH' enviroment varible not found,set Hascal compiler path to 'HPATH'"
                    )
                    exit(1)
        ...
        try:
            for statement in tree[1:]:
                if has_node(statement, 'function'):
                    HascalException(
                        "Error : You can't declere functions under another block"
                    )
                    return False
            return True
        except:
    ...
    def walk(self, node):
        ...
        if node[0] == 'declare' and node[1] == "no_equal":
            if node[3] in self.vars:
                HascalException(
                    f"Error : '{node[3]}' exists ,cannot redefine it")
                return ""
            elif node[3] in self.types:
                HascalException(
                    f"Error : '{node[3]}' defined as a type ,cannot redefine it as a variable"
                )
                return ""
            else:
        ...
        if node[0] == 'declare' and node[1] == "const":
            if node[2] in self.vars:
                HascalException(
                    f"Error : '{node[2]}' exists ,cannot redefine it")
                return ""
            elif node[2] in self.types:
                HascalException(
                    f"Error : '{node[2]}' defined as a type ,cannot redefine it as a variable"
                )
                return ""
            else:
        ...
        if node[0] == 'assign':
            if len(node[1]) == 1:
                ...
                if name in self.vars:
                    ...
                    HascalException(
                        f"Error : '{name}'is a type ,cannot change it")
                    return ""
                else:
            ...
            if sys.platform.startswith('win32'):
                if node[1] in self.imported:
                    ...
                    try:
                        ...
                        HascalException(
                            f"Error : cannot found '{tmp}' library. Are you missing a library ?"
                        )
        ...
        if node[0] == 'call':
            if self.exists_func(node[1]):
                if node[1] == "print":
                    return "writeln(%s)" % (', '.join(
                        self.walk(arg) for arg in node[2]))
                if node[1] == "write":
                    return "write(%s)" % (', '.join(
                        self.walk(arg) for arg in node[2]))
                else:
                    return "%s(%s)" % (node[1], ', '.join(
                        self.walk(arg) for arg in node[2]))
            else:
        ...
        # <expr>.<name>
        if node[0] == '.2':
            return '%s.%s[%s]' % (self.walk(
                node[1]), node[2], self.walk(node[3]))
        #--------------------------------------------
        if node[0] == 'string':
            return '"%s"' % node[1]
        if node[0] == 'bool':
