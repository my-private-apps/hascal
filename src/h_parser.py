...
class Parser(Parser):
    ...
    src_end = "\nreturn 0;}"
    #---------------------
    precedence = (('left', PLUS, MINUS), ('left', TIMES, DIVIDE), ('right',
                                                                   UMINUS))
    ...
    def statement(self, p):
        if sys.platform.startswith('win32'):
            ...
            try:
                ...
                print(
                    f"CompileError : cannot found {p.name} library. Are you missing a library ?"
                )
...
#---------------------------------------------#
    ...
    def var_assign(self, p):
        return str("{0} {1} = {{{2}}};".format(p.NAME1, p.NAME0,
                                               p.params_call))
    ...
    def const_assign(self, p):
        return "const {0} {1} = {{{2}}} ;".format(p.NAME1, p.NAME0,
                                                  p.arrays_list)
    ...
    def array_assigns(self, p):
        return "{0} {1} {2} = {3} ;".format(p.name, p.arrays, p.NAME,
                                            p.arrays_list)
    ...
    def if_stmt(self, p):
        return str("if({0}){{{1}}}else {{{2}}}".format(p.conditions,
                                                       p.in_statements0,
                                                       p.in_statements1))
    ...
    def if_stmt(self, p):
        return str("if({0}){{{1}}}else {2}".format(p.conditions,
                                                   p.in_statements, p.if_stmt))

    #--------------------------------
    @_('FOR name ASSIGN expr TO expr  LBC in_statements RBC')
    def loop_stmt(self, p):
        return str("for({0}={1};{2} <= {3};{4}++){{{5}}}".format(
            p.name, p.expr0, p.name, p.expr1, p.name, p.in_statements))
    ...
    def func_declare(self, p):
        return "{0} {1} ({2}) {{{3}}}".format(p.return_type, p.NAME, p.params,
                                              p.in_statements)
